...
checkpoint = 'ckpt_rn_transformer_8_4/checkpoint_flickr8k_5_cap_per_img_5_min_word_freq.pth.tar'
...
def evaluate(beam_size, max_length=15):
    ...
    for batch_idx, (image, caps, caplens, allcaps) in enumerate(
        ...
        encoder_out = encoder_out.reshape(1, -1, encoder_out.shape[-1]) 
        encoder_out = encoder_out.detach()
        logits = decoder(encoder_out, torch.tensor([word_map['<start>']], dtype=torch.int32).to(device).reshape(1, -1))
        ...
        # log_prob = torch.log(torch.nn.functional.softmax(logits.detach()[-1]))
        ...
        for itr in range(2, max_length):
            ...
            for i in range(k):
                ...
                encoder_out = encoder_out.reshape(1, -1, encoder_out.shape[-1]) 
                encoder_out = encoder_out.detach()
                logits = decoder(encoder_out, torch.tensor(targets[i]).to(device).view(1, -1))
                log_prob = torch.log(torch.nn.functional.softmax(logits.detach()[0][-1]))
                # log_prob = torch.log(torch.nn.functional.softmax(logits.detach()[-1]))
